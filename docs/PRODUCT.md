...
| 🤖 AI续写旋律           | `milestones/04-AI续写旋律.md`             | 📋 规划中 |
...
- 导出：将 sequencer 中的音符数据导出为标准 MIDI 文件
- 导入：解析外部 MIDI 文件，在钢琴卷帘中显示
- 支持多音轨、多通道的 MIDI 数据映射
...
- 模型：Minimax o3-mini（便宜、支持 JSON 输出）
- 流程：MIDI 数据 → Prompt → AI 返回 JSON → 转换为音符 → 追加到 sequencer
- UI：PianoRoll 工具栏添加 "AI 续写" 按钮 + 提示词输入弹窗
